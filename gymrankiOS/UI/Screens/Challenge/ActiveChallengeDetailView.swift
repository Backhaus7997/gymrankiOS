...
struct ActiveChallengeDetailView: View {
    ...
    var body: some View {
        ...
        .confirmationDialog("¿Abandonar desafío?", isPresented: $showConfirmCancel, titleVisibility: .visible) {
            Button("Abandonar", role: .destructive) {
                Task { await setStatus(UserChallengeStatus.cancelled) }
            }
            Button("Cancelar", role: .cancel) {}
        } message: {
            Text("Se marcará como abandonado y saldrá de Pendientes.")
        }
        .confirmationDialog("¿Marcar como completado?", isPresented: $showConfirmComplete, titleVisibility: .visible) {
            Button("Completado") {
                Task { await setStatus(UserChallengeStatus.completed) }
            }
            Button("Cancelar", role: .cancel) {}
        } message: {
            ...
            HStack(spacing: 12) {
                ...
                Button {
                    if canMarkCompleted {
                        ...
                        errorMessage = "Todavía no podés marcarlo como completado. Te faltan \(remainingDays) día(s)."
                    }
                } label: {
                    HStack(spacing: 10) {
                        if isLoading {
